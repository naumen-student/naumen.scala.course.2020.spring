...
object Exercises {
  ...
  def primeFactor(number: Int): Seq[Int] = {
    number match  {
      ...
      case 2 => Seq(2)
      case 3 => Seq(3)
      case _ => calculatePrimes(number)
    }
  }

  def calculatePrimes(number: Int): Seq[Int] = {
    val multipliers = ListBuffer[Int]()
    val sqrtNumber = Math.sqrt(number)
    var currentValue = number
    var currentMultiplier = 2
    while (currentValue > 1 && currentMultiplier <= sqrtNumber) {
      if (currentValue % currentMultiplier == 0) {
  ...
  */
  ...
  def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
    ballsArray.toSeq.sortBy(x => x._2._2 * ((4/3) * Math.PI * Math.pow(x._2._1, 3)))
      .foldLeft(Seq[String]())((x, y) => x :+ y._1)
  }

object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        ...
        def factorize(n: Int, d: Int = 2, prev: Int = 1, primes: Seq[Int] = Nil): Seq[Int] = {
            if (n == 1) return primes
            if (d * d > n) return primes :+ n
            if (n % d == 0)
                factorize(n / d, d, d, if (d == prev) primes else primes :+ d)
            else
                factorize(n, d + 1, d, primes)
        }
        factorize(number)
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] =
        ballsArray.mapValues(v => math.pow(v._1, 3) * v._2).toSeq.sortBy(_._2).map(_._1)
}
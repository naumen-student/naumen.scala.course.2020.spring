...
object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        ...
        var prime=true
        for ( divider <- 2 to number){
            if (number % divider == 0) {
                for (i<- 2 until divider){
                    if (divider % i == 0) {
                        prime=false
                    }
                }
    ...
    def sumByFunc(leftVec0: Vector2D, leftVec1: Vector2D, functionForVectors: (Vector2D,Vector2D) => Double, rightVec0: Vector2D, rightVec1: Vector2D): Double = {
        functionForVectors(leftVec0,leftVec1)+functionForVectors(rightVec0,rightVec1)
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        val result: Seq[String] = ballsArray.toSeq.sortBy{ x => (4 * java.lang.Math.PI * java.lang.Math.pow(x._2._1,3) * x._2._2) / 3}.map(x => x._1)
        result
    }

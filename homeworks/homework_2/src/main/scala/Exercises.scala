...
object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        def factorize(number: Int, divider: Int): Seq[Int] = {
            if (divider > Math.pow(number, 0.5))
                return Seq(number)

            number % divider match {
                case 0 => factorize(number / divider, divider) :+ divider
                case _ => factorize(number, divider + 1)
            }
        }
        factorize(number, 2).distinct.reverse
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ballsArray.toSeq.sortBy{
            b => Math.pow(b._2._1, 3) * Math.PI * 4 / 3 * b._2._2
        }.map(b => b._1)
    }

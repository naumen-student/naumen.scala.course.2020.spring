...
object Exercises {
    ...
    на 3 или на 5.*/
    /*Реализовать юнит-тесты в src/test/scala для данной функции.*/
    def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long = (iFrom to iTo).filter(v => v % 3 == 0 || v % 5 == 0).sum
    ...
    def primeFactor(number: Int): Seq[Int] = {
        @tailrec
        def divide(x: Int, y: Int): Int = if (x % y ==0 ) divide(x / y, y) else  x

        val (primeFactors, rest) = (2 to math.sqrt(number).toInt).foldLeft((Set.empty[Int], number)) {
            case ((factors, number), factor) =>
                if (number % factor == 0) (factors + factor, divide(number, factor)) else (factors, number)
        }

        if (rest != 1) (primeFactors + rest).toSeq
        else primeFactors.toSeq
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        def weight(r: Int, p: Double): Double = (4 / 3) * math.Pi * math.pow(r, 3) * p
        ballsArray.toList.sortBy { case (name, (r, p)) => weight(r, p) }.map(_._1)
    }

...
object Exercises {
    ...
    def divBy3Or7(iFrom: Int, iTo: Int): Seq[Int] = {
        (iFrom to iTo).filter(x => x % 3 == 0 || x % 7 == 0)
    }
    ...
    def primeFactor(number: Int): Seq[Int] = {
        @tailrec
        def inner(n: Int, a: Int, acc: Set[Int]): Set[Int] = a * a > n match {
            case false if n % a == 0 => inner(n/a, a, acc + a)
            case false => inner(n, a+1, acc)
            case _ => if (n > 1) acc + n else acc
        }

        inner(number, 2, Set.empty[Int]).toSeq
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] =
        ballsArray.mapValues(weightBall).toSeq.sortBy(_._2).map(_._1)

    def weightBall(ball: (Int, Double)):Double = ball match {
        case (radius, density) => 4/3 * Math.PI * math.pow(radius, 3) * density
    }

...
object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        var simpleNumbers = List(2)
        var res = List[Int]()
        if (number % 2 == 0) res = res :+ 2
        for (i <- 2 to number){
            if (number % i == 0 && simpleNumbers.forall(x => i % x != 0)){
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        Map(ballsArray.toSeq.sortBy(x => x._2._2 * (x._2._1 * x._2._1 * x._2._1 * Math.PI * 4/3)):_*).keys.toSeq
    }

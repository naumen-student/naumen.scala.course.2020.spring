object Exercises {
    ...
    def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long = {
        if (iFrom > iTo)
            throw new IllegalArgumentException("iFrom must be greater or equal than iTo")
        (iFrom to iTo)
    ...
    def primeFactor(number: Int): Seq[Int] = {
        ...
        if (dividend > 1) factors += dividend
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ballsArray
            .toList
            .sortBy(ball => 4 / 3 * ball._2._2 * java.lang.Math.PI * java.lang.Math.pow(ball._2._1, 3))
            .map(_._1)
    }
}
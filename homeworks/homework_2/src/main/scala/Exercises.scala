...
object Exercises {
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        var weightsMap: Map[String, Double] = Map[String, Double]()
        for ((name, (radius, density)) <- ballsArray) {
            weightsMap += (name -> density * 4 * java.lang.Math.PI * radius * radius * radius / 3)
        }
        val sorted = ListMap(weightsMap.toSeq.sortBy(_._2):_*)
        sorted.keys.toSeq
    }

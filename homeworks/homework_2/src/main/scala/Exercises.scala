...
object Exercises {
    ...
    на 3 или на 5.*/
    /*Реализовать юнит-тесты в src/test/scala для данной функции.*/
    def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long = iFrom to iTo filter(elem => elem % 3 == 0 || elem % 5 == 0) sum
    ...
    def primeFactor(number: Int): Seq[Int] = {
        val primeNumber: ListBuffer[Int] = ListBuffer()
        var divider: Int = 2
        var remains: Int = number
        while (divider * divider <= number) {
            if (remains % divider == 0)  {
                primeNumber += divider
                remains /= divider
            }
            else divider += 1
        }
        if (remains > 1) primeNumber += remains
        primeNumber.toSet.toSeq
    }
    ...
    def sumByFunc(leftVec0: Vector2D, leftVec1: Vector2D, vectorFunc: (Vector2D, Vector2D) => Double,
                  rightVec0: Vector2D, rightVec1: Vector2D) =
        vectorFunc(leftVec0, leftVec1) + vectorFunc(rightVec0, rightVec1)
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ballsArray.map(item => (item._1, 4 / 3 * Math.PI * Math.pow(item._2._1, 3) * item._2._2)).toSeq.sortBy(_._2)
          .map(item => item._1)
    }
}
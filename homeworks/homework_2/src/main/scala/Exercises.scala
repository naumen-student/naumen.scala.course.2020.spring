...
object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        @tailrec
        def addPrimeDelimiter(number: Int, delimiter: Int = 2, list: List[Int] = Nil): List[Int] = delimiter*delimiter > number match {
            case false if number % delimiter == 0 => addPrimeDelimiter(number / delimiter, delimiter, delimiter :: list)
            case false               => addPrimeDelimiter(number    , delimiter + 1, list)
            case true                => number :: list
        }
        addPrimeDelimiter(number).toSet.toSeq.sorted
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        val volumeCoefficient = Math.PI * 4 / 3

        def getBallMass(radius: Int, density: Double): Double = {
            volumeCoefficient * math.pow(radius, 3) * density
        }
        ballsArray.map{case (name, (radius, density)) =>
            name -> getBallMass(radius, density)
        }.toSeq.sortBy(nameAndMass => nameAndMass._2).map(nameAndMass => nameAndMass._1)
    }

...
object Exercises {
    ...
    def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long = {
        (iFrom to iTo).foldLeft(0) {
            case (result, i) => if (i % 3 == 0 || i % 5 == 0) result + i else result
        }
    }
    ...
    def primeFactor(number: Int): Seq[Int] = {
        @tailrec
        def factorize(number: Int, iFrom: Int = 2, result: Set[Int] = Set.empty): Set[Int] = {
            iFrom * iFrom > number match {
                case false if number % iFrom == 0 => factorize(number / iFrom, iFrom, result + iFrom)
                case false                  => factorize(number, iFrom + 1, result)
                case true                   => result + number

            }
        }
        factorize(number).toSeq
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ballsArray.toSeq.sortBy(item => math.pow(item._2._1, 3) * item._2._2).map(_._1)
    }

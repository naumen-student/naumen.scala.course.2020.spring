object Exercises {
    ...
    на 3 или на 5.*/
    /*Реализовать юнит-тесты в src/test/scala для данной функции.*/
    def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long = List.range(iFrom, iTo + 1).filter(x => x % 3 == 0 || x % 5 == 0).sum
    ...
    def primeFactor(number: Int): Seq[Int] = {
        def smallestFactor(num: Int): Int = {
            for {i <- 2 to num if num % i == 0} return i
            num
        }
        var factors: Seq[Int] = Seq()
        var current = number
        while (current != 1) {
            val sf = smallestFactor(current)
            if (!factors.contains(sf)) factors :+= sf
            current /= sf
        }
        factors
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ballsArray.toSeq
          .sortBy(ball => 4 / 3 * math.pow(ball._2._1, 3) * math.Pi * ball._2._2)
          .map(ball => ball._1)
    }

object Exercises {
  ...
  private val primes = {
    val a = collection.mutable.ArrayBuffer.fill(1000)(true)
    a(0) = false
    a(1) = false
    for (i <- 2 until a.length ) {
      if (a(i))
        for (j <- i * i until a.length by i)
          a(j) = false
    }
  ...
  def primeFactor(number: Int): Seq[Int] = {
    var result = Seq[Int](1)
    var temp = number
    while(temp != 1){
      val den : Int = primes.find(x=>temp % x == 0).getOrElse(1)
      temp = temp/den
      result = result:+ den
      }
    result
  ...
  */
  ...
  def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = ballsArray
    .map(x=>x._1 -> getMass(x._2._1,x._2._2))
    .toSeq
    .sortBy(x=>x._2)
    .map(x=>x._1)


  private def getMass(radius: Int, density : Double) : Double = 4.0/3.0 * java.lang.Math.PI * radius * radius * radius * density
}
...
object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        @tailrec
        def findDenominator(number: Int, denom: Int = 2, res: Set[Int] = Set.empty[Int]): Set[Int] = denom * denom <= number match {
            case true if number % denom == 0 => findDenominator(number/denom, denom, res+denom)
            case true => findDenominator(number, denom+1, res)
            case false => if (number > 1) res + number else res
        }

        findDenominator(number).toSeq
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        val coeff = java.lang.Math.PI * 4/3

        def calculateTheMass(ball: (Int, Double)): Double ={
            coeff * java.lang.Math.pow(ball._1, 3) * ball._2
        }

        ballsArray.mapValues(calculateTheMass).toSeq.sortBy(_._2).map(x => x._1)
    }

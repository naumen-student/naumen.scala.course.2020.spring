object Exercises {
    ...
    на 3 или на 5.*/
    /*Реализовать юнит-тесты в src/test/scala для данной функции.*/
    def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long = (iFrom to iTo).filter((i: Int) => i % 3 == 0 || i % 5 == 0).sum
    ...
    def primeFactor(number: Int): Seq[Int] = {
        val res = scala.collection.mutable.Set[Int]()
        var multiplier = 2
        var x = number
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = ballsArray.toSeq
      .sortBy({ case (name, extension) => (extension._2 * 4 * Math.PI * Math.pow(extension._1, 3)) / 3 }).map(_._1)

...
object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        @tailrec
        def factorize(number: Int, divider: Int = 2, dividers: Seq[Int] = Nil): Seq[Int] =
            if (number == 1)
                dividers
            else if (number % divider == 0)
                factorize(number / divider, divider, dividers :+ divider)
            else
                factorize(number, divider + 1, dividers)

        factorize(number).distinct
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] =
        ballsArray.toSeq.sortBy{
            case (_, (radius, density)) => density * 4 / 3 * Math.pow(radius, 3) * Math.PI
        }.map{
            case (name, _) => name
        }
}
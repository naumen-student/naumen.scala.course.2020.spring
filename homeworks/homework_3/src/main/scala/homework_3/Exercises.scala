...
class MyIterable(val x: Int, val y: Int) extends Iterable[Int] {
...
object Exercises {
  ...
  def prettyBooleanFormatter2(x: Any): String = {
    x match {
      case bool: Boolean =>
        return if (bool) "правда" else "ложь";
      case _ =>
    }
    x.toString
  }
  ...
  def prettyBooleanFormatter3(x: Any): String = {
    val cl = x.getClass
    if (cl.getName == "java.lang.Boolean") {
      return if (x.asInstanceOf[Boolean]) "правда" else "ложь";
    }
    return x.toString
  }
  ...
  def sum2(x: Int, y: Int): Int = sumIntegers(new MyIterable(x, y))

  def sum3(x: Int, y: Int): Int = sumIntegers(Array(x,y).toIterable)

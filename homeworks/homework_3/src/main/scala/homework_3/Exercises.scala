...
object Exercises {
    ...
    def max2(xs: Seq[Int]): Seq[Int] = xs.filter(_ == xs.max).take(1)
    ...
    def sum3(x: Int, y: Int): Int = sumIntegers(IncrementNumbers(x, y))

    case class IncrementNumbers(x: Int, y: Int) extends Iterable[Int] {
        override def iterator: Iterator[Int] = Iterator(x + 1, y + 1)
    }
}

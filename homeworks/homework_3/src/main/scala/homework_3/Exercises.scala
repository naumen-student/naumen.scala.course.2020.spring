...
object Exercises {
    ...
    def max1(xs: Seq[Int]): Int = {
        if (xs.isEmpty) ??? else xs.max
    }

    def max2(xs: Seq[Int]): Seq[Int] = {
        if (xs.isEmpty) xs else Seq(xs.max)
    }
    ...
    def sum2(x: Int, y: Int): Int = sumIntegers(NumbersCollection(x,y))
    def sum3(x: Int, y: Int): Int = sumIntegers(x :: y :: Nil)

    case class NumbersCollection(x: Int, y: Int) extends Iterable[Int] {
